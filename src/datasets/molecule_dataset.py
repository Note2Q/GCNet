...
class AliasTable(nn.Module):
    def __init__(self, probs):
        ...
        #num_element ： node/task数
        self.num_element = len(probs) #132
    ...
    def build(self, probs):
        with torch.no_grad():
            ...
            index = torch.arange(len(probs))#[0, 1, ---, task-1]
...
class PPI_dataset(InMemoryDataset):
    def __init__(self, args, string_threshold, neg_sample_size, neg_sample_exponent):
        ...
        edge_list = []
        self.neg_sample_size = neg_sample_size #5
        ...
        for i in range(num_tasks):
            for j in range(num_tasks):
                if i==j:
                    ...
                    num_full_edge += 1
        self.edge_list_full = torch.LongTensor(edge_list_full)



        degree = [0 for _ in range(num_tasks)]
        for u in self.edge_list_full[:, 0]:
            degree[u] += 1
        self.degree = torch.FloatTensor(degree)
        self.drug_table = AliasTable(self.degree ** self.neg_sample_exponent)
        print(self.drug_table)
        return

    def __len__(self):
        return len(self.edge_list_full)

    def __getitem__(self, idx):
        edge = self.edge_list_full[idx]
        node0, node1 = edge
...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in tqdm(range(len(rdkit_mol_list))):
            ...
            # y is task label list for each mol
