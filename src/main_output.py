...
from models import (
    GNN_graphpred,
    GCNNet, TaskEmbeddingModel,
    MoleculeTaskPredictionModel, MoleculeTaskTaskPredictionModel, PairwiseTaskPredictionModel,
    GNN_Energy_Model_1st,
    GNN_Energy_Model_2nd,
    NCE_C_Parameter
)
from configure import *
from gnn_causal_ebm import *
from gnn_causal_ebm_support import *


def get_task_representation(task_embedding_model, gcn_model, task_X):
    task_repr = task_embedding_model(task_X)
    if args.use_GCN:
        task_repr = gcn_model(task_repr, task_edge)
    return task_repr
...
def train_PPI(args, device, loader, task_relation_optimizer):
    ...
    for step, batch in enumerate(loader):
        task_repr = get_task_representation(task_embedding_model, gcn_model, task_X)
        ...
        task_acc = torch.sum((torch.sigmoid(pred) >= 0.5) == (label > 0.5)).float() / (
                (1 + args.neg_sample_size) * N)
...
def train_MTL(args, model, readout_model, device, loader, optimizer):
    ...
    for step, batch in enumerate(loader):
        ...
        y_true = batch.y.view(y_pred.shape).to(torch.float64)  # batch.y is mol's attri-label
        optimizer.zero_grad()
...
def train(args, device, loader, optimizer):
    ...
    for step, batch in enumerate(loader):
        ...
        B = len(graph_repr)
        y_true = batch.y.view(B, args.num_tasks).to(torch.float64)  # B * num_tasks
        ...
        if args.energy_function in ['energy_function_CD_AA', 'energy_function_CD_GS']:
            _, _, first_order_energy_loss, second_order_energy_loss = energy_function(
                ...
                first_order_label_weights=first_order_label_weights,
                second_order_label_weights=second_order_label_weights,
            ...
            'energy_function_output',
        ]:
            energy_loss = energy_function(
                first_order_prediction_model=first_order_prediction_model,
                second_order_prediction_model=second_order_prediction_model,
                GNN_energy_model=GNN_energy_model, adj=adj,
                prediction_function=prediction_function,
                graph_repr=graph_repr, task_repr=task_repr, y_true=y_true, task_edge=task_edge,
                NCE_C_param=NCE_C_param, prior_prediction=prior_prediction, id=batch.id,
                mask=mask, use_trivial=args.use_trivial, DAG=DAG, A_causal=A_causal,
            ...
            _, _, first_order_energy_loss, second_order_energy_loss, \
            first_order_energy_loss_trivial, second_order_energy_loss_trivial \
                = energy_function(
                first_order_prediction_model=first_order_prediction_model,
                second_order_prediction_model=second_order_prediction_model,
    ...
    print('energy loss: {:.5f}'.format(first_order_energy_loss_accum / len(loader)))
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(loader):
            ...
            if args.inference_function in [
                'CGMTL_inference_output',
            ]:
                # B,T,1
                y_pred = inference_function(
                    first_order_prediction_model=first_order_prediction_model,
                    second_order_prediction_model=second_order_prediction_model,
                    GNN_energy_model=GNN_energy_model, mask=mask, DAG=DAG,
                    prediction_function=prediction_function, A_causal=A_causal, adj=adj,
                    graph_repr=graph_repr, task_repr=task_repr, y_true=y_true, task_edge=task_edge,
                    prior_prediction=prior_prediction, id=batch.id,
                    prior_prediction_logits=prior_prediction_logits,
                    args=args)
                ...
                y_pred = inference_function(
                    first_order_prediction_model=first_order_prediction_model,
                    second_order_prediction_model=second_order_prediction_model,
                    energy_function=energy_function, adj=adj,
                    graph_repr=graph_repr, task_repr=task_repr, y_true=y_true, task_edge=task_edge,
                    first_order_label_weights=first_order_label_weights,
                    second_order_label_weights=second_order_label_weights,
                    args=args)
            ...
            for step, batch in enumerate(loader):
                ...
                if args.inference_function in [
                    ...
                    y_pred = inference_function(
                        ...
                        second_order_prediction_model=second_order_prediction_model,
                        GNN_energy_model=GNN_energy_model,
                        prediction_function=prediction_function, adj=adj, A_causal=A_causal,
                        graph_repr=graph_repr, task_repr=task_repr, y_true=y_true, task_edge=task_edge,
                        prior_prediction=prior_prediction, id=batch.id, mask=mask,
                        prior_prediction_logits=prior_prediction_logits,
                        args=args)
                else:
                    y_pred = inference_function(
...
def extract_prior_distribution_from_pretrained_model(args):
    def load(dir_, mode):
        print('{}/{}.npz'.format(dir_, mode))
        data = np.load('{}/{}.npz'.format(dir_, mode))
        y_pred_list = data['y_pred_list']
        id_list = data['id_list']
        return y_pred_list, id_list

    filling_mode = args.filling_mode
    dir_ = '../feature_checkpoint/{}/{}/{}'.format(filling_mode, args.dataset, args.seed)
    y_pred_train, id_train = load(dir_, 'model_train')
    y_pred_valid, id_valid = load(dir_, 'model_valid')
    y_pred_test, id_test = load(dir_, 'model_test')
...
def load_a(args):
    dir_ = '../feature_checkpoint/gnn/{}/{}'.format(args.dataset, args.seed)
    data = np.load('{}/{}.npz'.format(dir_, 'model_adj'))
    a_causal = data['a_causal']
    print("a_causal{}".format(a_causal.shape))
    return a_causal
...
def save_model(save_best=False):
    if not args.output_model_file == '':
        ...
        if args.use_GCN:
            saver['gcn_model'] = gcn_model.state_dict()
...
if __name__ == '__main__':
    ...
    assert args.mtl_method == 'causal_ebm'
    ...
    elif args.energy_function == 'energy_function_output':
    ...
    if args.inference_function == 'feature_inference':
    ...
    elif args.inference_function == 'CGMTL_inference_output':
        inference_function = output_GS_inference
    else:
    ...
    ppi_dataloader = DataLoader(ppi_dataset, batch_size=128, shuffle=True, num_workers=args.num_workers)

    ########## Set up task edge list ##########

    task_edge = copy.deepcopy(ppi_dataset.edge_list.transpose(0, 1)).to(device)
    A_causal = torch.tensor(load_a(args), device=device, requires_grad=False)
    adj = torch.zeros(args.num_tasks, args.num_tasks).to(device)
    for i in range(task_edge.size()[1]):
        node1 = task_edge[0][i]
        node2 = task_edge[1][i]
    adj = torch.triu(adj)
    ...
    if args.energy_function in ['energy_function_CD_AA']:
        ...
        second_order_prediction_model = MoleculeTaskTaskPredictionModel(
            args.emb_dim, args.task_emb_dim, output_dim=args.ebm_GNN_dim, batch_norm=args.batch_norm).to(device)
    if args.with_edge_attention:
        mask = mask_A(args.task_emb_dim, args).to(device)
    DAG = DAG_model(input_dim=2, adj=adj, args=args, bias=True).to(device)
    NCE_C_param = None

    if args.energy_function == 'energy_function_output':
    ...
    if args.energy_function in ['energy_function_GNN_CE_1st_order']:
        if args.gnn_energy_model == 'GNN_Energy_Model_1st_Order_01':
            GNN_energy_model = GNN_Energy_Model_1st_Order_01(
                ebm_GNN_dim=args.ebm_GNN_dim, ebm_GNN_layer_num=args.ebm_GNN_layer_num, concat=args.ebm_GNN_use_concat,
    ...
    if args.energy_function in [
        'energy_function_GNN_CE_2nd_order', 'energy_function_output',
        'energy_function_GNN_EBM_CD_GS', 'energy_function_GNN_EBM_CE_2nd_order_Binary_Task'
    ]:
        if args.gnn_energy_model == 'GNN_Energy_Model_2nd':
            GNN_energy_model = GNN_Energy_Model_2nd(
                ebm_GNN_dim=args.ebm_GNN_dim, ebm_GNN_layer_num=args.ebm_GNN_layer_num,
                concat=args.ebm_GNN_use_concat).to(device)
        else:
            raise ValueError('GNN Energy Model {} not included.'.format(args.gnn_energy_model))
        print('GNN_energy_model\n', GNN_energy_model)
    ...
    gcn_model = None
    if args.use_GCN:
        gcn_model = GCNNet(embedding_dim=args.task_emb_dim, hidden_dim=args.task_emb_dim,
                           dropout=args.kg_dropout_ratio).to(device)
    ...
    '''
    first_order_label_weights, second_order_label_weights = extract_amortized_task_label_weights(train_dataloader,
                                                                                                 task_edge, device,
                                                                                                 args)
    ...
    if args.use_GCN:
        model_param_group.append({'params': gcn_model.parameters()})
    if NCE_C_param is not None:
        model_param_group.append({'params': NCE_C_param.parameters(), 'lr': args.lr * 10})
    if args.with_edge_attention is not None:
        model_param_group.append(({'params': mask.parameters()}))
    model_param_group.append(({'params': DAG.parameters()}))
    print('model_param_group\n', model_param_group)
    optimizer = optim.Adam(model_param_group, lr=args.lr, weight_decay=args.decay)

    # Loading prior data
    prior_prediction, prior_prediction_logits = None, None
    if args.filling_mode in ['mtl', 'mtl_uncertainty', 'gradnorm', 'dwa', 'lbtw', 'mtl_task', 'mtl_task_KG', 'gnn',
                             'ebm']:
    ...
    if gcn_model is not None:
        parameters += list(gcn_model.parameters())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.eval_every_n_epochs == 0:
            ...
            test_roc, y_true_list, y_pred_list, id_list = eval(args, model, device, test_dataloader, 'test')

            if valid_roc > best_valid_roc:
                best_valid_roc = valid_roc
                best_test_roc = test_roc
                save_model(save_best=True)
                _, y_true_list, y_pred_list, id_list = eval(args, model, device, train_dataloader, 'train')

                adj_best = adj.cpu().detach().numpy()
                plt.matshow(adj_best, cmap=plt.cm.Blues)
                plt.title("matrix A")
                plt.savefig('{}_a_{}.png'.format(args.output_model_file, epoch))
                print("adj img saved")
                filename = '{}_adj'.format(args.output_model_file)
                adj_new = adj.cpu().detach().numpy()
                np.savez(filename, adj=adj_new)

                filename = '{}_train'.format(args.output_model_file)
                print('save to \t', filename)
                np.savez(filename, y_true_list=y_true_list, y_pred_list=y_pred_list, id_list=id_list)
                _, y_true_list, y_pred_list, id_list = eval(args, model, device, valid_dataloader, 'valid')
                filename = '{}_valid'.format(args.output_model_file)
                print('save to \t', filename)
                np.savez(filename, y_true_list=y_true_list, y_pred_list=y_pred_list, id_list=id_list)
                ...
                if prior_prediction is not None:
                    ...
                    np.savez(filename, prior_prediction=prior_prediction.cpu().numpy(),
                             prior_prediction_logits=prior_prediction_logits.cpu().numpy())

        if epoch >= args.filling_missing_data_fine_tuned_epoch and epoch % args.eval_every_n_epochs == 0 and \
                args.filling_mode in ['mtl', 'mtl_uncertainty', 'gradnorm', 'dwa', 'lbtw', 'mtl_task', 'mtl_task_KG',
                                      'gnn', 'ebm']:
            print('change missing data filling mode from {} to current best model'.format(args.filling_mode))
            prior_prediction, prior_prediction_logits = extract_prior_distribution_from_current_model(args,
                                                                                                      [train_dataloader,
                                                                                                       valid_dataloader,
                                                                                                       test_dataloader])
    ...
    if valid_roc > best_valid_roc:
        best_valid_roc = valid_roc
        save_model(save_best=True)
        filename = 'test'.format(args.output_model_file)
        print('save to \t', filename)
        np.savez(filename, y_true_list=y_true_list, y_pred_list=y_pred_list, id_list=id_list)
        if prior_prediction is not None:
            filename = '{}_prior'.format(args.output_model_file)
            np.savez(filename, prior_prediction=prior_prediction.cpu().numpy(),
                     prior_prediction_logits=prior_prediction_logits.cpu().numpy())
    print("=====test_roc_for_best_valid=====")
    print("best_test_roc{}".format(best_test_roc))
