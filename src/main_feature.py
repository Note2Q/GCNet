...
def train(args, device, loader, optimizer):
    ...
    for step, batch in enumerate(loader):
        ...
        feature_loss,A_causal = energy_function(
            ...
            mask=mask, use_trivial=args.use_trivial,
            prior_prediction_logits=prior_prediction_logits,
            args=args)
        feature_loss_accum += feature_loss.detach().item()

        optimizer.zero_grad()
        feature_loss.backward()
        optimizer.step()
    print('feature loss: {:.5f}'.format(feature_loss_accum / len(loader)))
    return A_causal
        ...
        for step, batch in enumerate(loader):
            ...
            if args.inference_function in [
                ...
                y_pred = inference_function(
                    first_order_prediction_model=first_order_prediction_model,
                    second_order_prediction_model=second_order_prediction_model,
                    GNN_energy_model=GNN_energy_model, mask=mask,
                    prediction_function=prediction_function,
                    graph_repr=graph_repr, task_repr=task_repr, y_true=y_true, task_edge=task_edge,
...
if __name__ == '__main__':
    ...
    if args.inference_function == 'CGMTL_inference_feature':
        inference_function = feature_inference
    ...
    if args.energy_function in ['energy_function_feature']:
    ...
    if args.energy_function in ['energy_function_feature']:
        if args.gnn_energy_model == 'GNN_Energy_Model_1st':
    ...
    if args.energy_function in ['energy_function_output']:
    ...
    task_edge_nofc = copy.deepcopy(ppi_dataset.edge_list.transpose(0, 1)).to(device) # 2 * M
    ...
    if args.filling_mode in ['mtl', 'mtl_uncertainty', 'gradnorm', 'dwa', 'lbtw', 'mtl_task',
                                          'mtl_task_KG', 'gnn', 'ebm']:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        start_time = time.time()
        a_causal = train(args, device, train_dataloader, optimizer).detach().cpu().numpy()
        if epoch % args.eval_every_n_epochs == 0:
            ...
            if valid_roc > best_valid_roc:
                ...
                np.savez(filename, a_causal=a_causal)
                print("adj saved")
