...
from main_output,main_feature import *
        ...
        for step, batch in enumerate(loader):
            ...
            if args.inference_function in [
                ...
                y_pred = inference_function(
                    ...
                    prediction_function=prediction_function, A_causual=A_causual, adj=adj,
...
def load_a(args):
    dir_ = '../gnn/{}/{}'.format(args.dataset, args.seed)
    data = np.load('{}/{}.npz'.format(dir_, 'model_adj'))
    a_causual = data['a_causual']
    return a_causual

def load_model(output_model_file):
    ckpt = torch.load('../{}_best.pth'.format(output_model_file))
...
if __name__ == '__main__':
    ...
    if args.energy_function == 'energy_function_output':  # train for SGNN_EMB
